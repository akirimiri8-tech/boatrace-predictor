...
REM See run_daily_predict.bat for why this is needed.
...
start "Boatrace Predictor - Today's Picks" cmd /k powershell -NoLogo -NoProfile -Command ^
    "$env:PLAYWRIGHT_BROWSERS_PATH='0'; & '.venv\Scripts\python.exe' -X utf8 scripts\daily_predict.py 2>&1 | Tee-Object -FilePath 'logs\daily_predict.log' -Append"
